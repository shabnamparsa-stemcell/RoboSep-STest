...
class TestTube(unittest.TestCase):
	...
	self.failUnless(self.tube.getLabel() == self.ID, 'Label test')

    def test_startVolume(self):
	self.failUnless(self.tube.getVolume() == self.startVol, 'Initial volume test')

    def test_maxVolume(self):
	self.failUnless(self.tube.getMaxVolume() == self.maxVol, 'Maximum volume test')

    def test_deadVolume(self):
	self.failUnless(self.tube.getDeadVolume() == self.deadVol, 'Dead volume test')

    def test_addVolume(self):
	addedVolume = 1000
	self.tube.addVolume(addedVolume)
	newVol = self.startVol + addedVolume
	self.failUnless(self.tube.getVolume() == newVol, 'Add volume test')
	...
	self.failUnless(self.tube.getVolume() == newVolume, 'Remove volume test')
	...
	self.failUnless(self.tube.getVolume() == 0, 'Remove too much volume')
	...
	self.failUnless(self.tube.getMeniscus() == self.tubeHeight/2.0)

    def test_meniscusWhenAdding(self):
	# Test meniscus height check on adding
	self.tube.removeVolume(self.tube.getVolume())
	self.failUnless(self.tube.getMeniscusAfterAdding(self.tube.getMaxVolume()) == self.tubeHeight)

    def test_meniscusWhenRemoving(self):
	# Test meniscus height check on removing
	self.tube.removeVolume(self.tube.getVolume())
	self.tube.addVolume(self.tube.getMaxVolume())
	self.failUnless(self.tube.getMeniscusAfterRemoving(self.tube.getMaxVolume()) == 0.0)

    def test_basePosition(self):
	# Test base position
	self.failUnless(self.tube.getBasePosition() == self.tubeBase)

    def test_tubeHeight(self):
	# Test tubeHeight
	self.failUnless(self.tube.getMaxHeight() == self.tubeHeight)

    def test_empty(self):
	startVol = 432
	self.tube = Tube(1, self.ID, self.geometry, startVol)
	self.failUnless(self.tube.getVolume() == startVol, 'Testing starting volume')
	self.tube.empty()
	self.failUnless(self.tube.getVolume() == 0, 'Testing empty tube')
	...
	for volume in (10, 100, 1000, self.maxVol):
	    ...
	    self.failUnless(self.tube.getVolume() == volume, "New set volume = %0.1f" % (volume))
	...
	tube = Tube(quad, self.ID, self.geometry, startVol)
        s = str(tube)
        self.failUnless(s == "5 mL tube [Q%d]: %d.0 of 5000.0 uL" % (quad, startVol), 'Testing str()')
