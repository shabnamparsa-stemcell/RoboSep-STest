...
class TestProtocolManager(unittest.TestCase):
    ...
    def test_checkForFiles(self):
        self.assertTrue(self.pm.getNumberOfProtocols() > 0, 'Need files for these tests')
    ...
    def test_database(self):
        if self.pm.usedDB():
            ...
            self.assertTrue(status == 0, 'Unable to delete protocol database')
            newPM = ProtocolManager(tesla.config.PROTOCOL_DIR)
            self.assertFalse(newPM.usedDB(), 'Testing that we did not use the database')
            anotherPM = ProtocolManager(tesla.config.PROTOCOL_DIR)
            self.assertTrue(anotherPM.usedDB(), 'Testing that we have used the database')
    ...
    def test_filesAreEqual(self):
        ...
        self.assertTrue(self.pm.filesAreEqual(f1, f2), 'Testing files are equal')

    def test_getProtocols(self):
        protocolList = self.pm.getProtocols()
        self.assertTrue(type(protocolList) == type([]), 'Test getProtocols() return type')
        self.assertTrue(len(protocolList) > 0, 'Need protocols for these tests')
        self.assertTrue(isinstance(protocolList[0], Protocol), 'Testing returned protocol')

    def test_getProtocolsForClient(self):
        protocolList = self.pm.getProtocols()        
        clientList = self.pm.getProtocolsForClient()
        self.assertTrue(len(clientList) > 0, 'Testing presence of client protocols')
        self.assertTrue(len(protocolList) == len(clientList), 'Testing length of client list')
        self.assertTrue(isinstance(clientList[0], ClientProtocol), 'Testing returned client protocol')
    ...
    def test_findProtocolsByRegexp(self):
        ...
        self.assertTrue(matches == protocols, "Testing regexp finding for %s" % (pattern))

        singleMatch = self.pm.findProtocolsByRegexp('Shutdown')
        self.assertTrue(len(singleMatch) == 1, 'Testing for single regexp hit')
    ...
    def test_findNoProtocolsByRegexp(self):
        ...
        self.assertTrue(matches == [], 'Testing regexp finding with no results')

    def test_findProtocolsByType(self):
        protocols = self.pm.findProtocolsByType('Maintenance')
        self.assertTrue(type(protocols) == type([]), 'Testing returned type')
        for p in protocols:
            self.assertTrue(p.type == 'Maintenance', 'Testing protocol type')

    def test_findNoProtocolsByType(self):
        protocols = self.pm.findProtocolsByType('SILLY_TYPE')
        self.assertTrue(len(protocols) == 0, 'Testing bad protocol-find type')
    ...
    def test_volumeInformation(self):
        ...
        self.assertTrue(type(pcList) == type([]), 'Testing consumables types')
        
        pc = pcList[0]
        self.assertTrue(isinstance(pc, ProtocolConsumable), 'Testing ProtocolConsumable type')

        protocolQuadrantsNumber = protocol.numQuadrants
        self.assertTrue(protocolQuadrantsNumber in range(1,5), 'Ensuring quadrant number is okay')
        self.assertTrue(numConsumables == protocolQuadrantsNumber, 'Testing # quadrants')
    ...
    def test_isSeparationProtocol(self):
        ...
        self.assertFalse(self.pm.isSeparationProtocol(mainP1), \
        ...
        for protocol in (sepP1,):
            self.assertTrue(self.pm.isSeparationProtocol(protocol.ID), \
                    "Testing %s-type protocol" % (protocol.type))

    def test_isSampleVolumeValidForProtocol(self):
        # Test a "safe" volume
        protocol = self.pm.findProtocolsByRegexp('Generic')[0]
        self.assertTrue(self.pm.isSampleVolumeValidForProtocol(protocol, 1000))

        # This should fail
        self.assertFalse(self.pm.isSampleVolumeValidForProtocol(protocol, -123))

        # Now test with an ID, rather than a protocol -- we should cope
        id = protocol.ID
        self.assertTrue(self.pm.isSampleVolumeValidForProtocol(id, 1000))
    ...
    def test_isSampleVolumeValidForProtocol_forMaintenance(self):
        ...
        vol = 1000
        mainP1 = self.pm.findProtocolsByType('Maintenance')[0]
        self.assertTrue(self.pm.isSampleVolumeValidForProtocol(mainP1, vol))
