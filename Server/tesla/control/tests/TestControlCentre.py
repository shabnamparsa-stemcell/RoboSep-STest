...
class TestCentre(unittest.TestCase):
	...
	self.failUnless(self.cc.noSchedule(), 'Testing no schedule')

    def test_reset(self):
	self.cc.resetInstrument()
	self.failUnless(self.cc.getState() == 'RESET', 'Testing state transition to RESET')
	...
	self.failUnless(self.cc.getState() == 'RESET', 'Testing state transition to RESET')
        ...
        self.failUnless(newState == 'SHUTDOWN', "Testing shutdown state transition (got %s)" % (newState))

    def test_getProtocols(self):
	pro = self.cc.getProtocols()
	self.failUnless(type(pro) == type([]), 'Testing protocols container')
	self.failUnless(len(pro) > 0, 'Testing that we have protocols')
	self.failUnless(isinstance(pro[0], ClientProtocol))

    def test_expandMsg_Status(self):
	msg0 = self.cc.expandMsg(self.cc.statusTable, 'TSC0002')
	self.failUnless(msg0 == 'INIT', 'Testing zero-argument status message')

	msg3 = self.cc.expandMsg(self.cc.statusTable, 'TSC9999', 9999, 'Gaiman', 1602)
	self.failUnless(msg3 == 'Status code 9999 testing Gaiman 1602', \
		'Testing three-argument status message')

	# This one should fail
	badMsg = self.cc.expandMsg(self.cc.statusTable, 'FOOBAR')
	self.failIf(badMsg.find('LOOKUP FAILURE') == -1, 'Testing bad lookup code')

    def test_expandMsg_Error0(self):
        # Test a 0-parameter error message
        msg0 = self.cc.expandMsg(self.cc.errorTable, 'TEC1000')
	self.failUnless(msg0 == 'Placeholder.', 'Testing zero-argument error message')

    def test_expandMsg_Error2(self):
        # Test a 2-parameter error message
	msg2 = self.cc.expandMsg(self.cc.errorTable, 'TEC9999', 42, 1967)
	self.failUnless(msg2 == 'Instrument interface error testing 42 1967.', \
	...
	self.failUnless(len(samples) == numSamples)
	filtered = self.cc.filterSamples(samples)
	self.failUnless(len(filtered) == 4, 'Testing filtered separation samples')
	...
	self.failUnless(len(samples) == numSamples)
	filtered = self.cc.filterSamples(samples)
	self.failUnless(len(filtered) == 1, 'Testing filtered maintenance samples')
	...
	self.failUnless(len(samples) == numSamples)
	filtered = self.cc.filterSamples(samples)
	self.failUnless(len(filtered) == 1, 'Testing filtered mixed samples')
	...
	self.failIf(self.cc.noSchedule(), 'Testing for successful schedule')
	
	# Now test the actual schedule
	schedule = self.cc.scheduler.getSchedule()
	self.failIf(schedule == None, 'Testing schedule')
	self.failUnless(isinstance(schedule, Schedule), 'Testing schedule class')

	# Do we have the right number of scheduled workflows?
	realProtocol = self.cc.pm.getProtocol(protocol1.ID)
	self.failUnless(realProtocol, 'Testing retrieval of instrument-level protocol')
	numCmds = numSamples * len(realProtocol.getCommands())
	numWorkflows = len(schedule.getWorkflows())
	self.failUnless(numCmds == numWorkflows, 'Testing number of scheduled workflows')
	
	# Test each workflow
	for wf in schedule.getWorkflows():
	    cmd, startTime = wf.getWorkflowAndTime()
	    self.failUnless(type(cmd) == type(''), 'Testing that cmd is a string')
	    self.failUnless(type(startTime) == type(1), 'Testing that startTime is a number')
	    self.failUnless(startTime >= 0, 'Testing that startTime is sensible')
	...
	samples, protocol1 = self.getSamplesAndProtocol(numSamples)
	etc = self.cc.scheduleRun(samples)
	self.failUnless(self.cc.noSchedule(), 'Testing for unsuccessful schedule')
	
	# Now test the actual schedule
	schedule = self.cc.scheduler.getSchedule()
	self.failUnless(schedule == None, 'Testing for empty schedule')
	...
	samples, protocol1 = self.getSamplesAndProtocol(numSamples)
        ...
        for duration in durations:
            if duration - duration0 > threshold:
                outOfRangeCount += 1
        self.failUnless(outOfRangeCount == 0, "Testing duration of repeat schedules (%d failed)" % (outOfRangeCount))
