...
namespace Tesla.Separator
{
	...
	public interface ISeparator 
	{
        ...
        int     InitScanVialBarcode(bool freeaxis);         //CWJ
        bool    AbortScanVialBarcode();                     //twong
        bool    IsBarcodeScanningDone();                    //twong
        int     ScanVialBarcodeAt(int Quadrant, int Vial);  //CWJ
        string  GetVialBarcodeAt(int Quadrant, int Vial);   //CWJ

        string  GetEndOfRunXMLFullFileName();               // 09-25-2013 - Sunny added

        int     SetIgnoreLidSensor(int sw);                 //CWJ
        bool    GetIgnoreLidSensor();

        int     SetIgnoreHydraulicSensor(int sw);           //CWJ
        bool    GetIgnoreHydraulicSensor();
        ...
        int     ConvertIdToSampleIndex(int id);                             

        void    StartOrcaTCPServer();
        void    StopOrcaTCPServer();
        void    BroadcastOrcaTCPServer(bool status);

        void CloseOrcaLogFileSystem();

        bool Ping();
        XmlRpcProtocolCommand[] getProtocolCommandList(int protocolIdx);
        bool IsSchedulerBusy();
        bool IsISeparatorBusy();
		[OneWay]
		void ReloadProtocols();

        string isSharingComboValid(string batchRunUserId, bool isSharing, int[] sharedSectorsTranslation, int[] sharedSectorsProtocolIndex);
