    ...
    public class BarcodeListView : DragScrollListView2
    {
        ...
        public BarcodeListView() : base()
        {
            ...
            for (int i = 0; i <= 3; i++)
            {
                int number = i + 1;
#if false
                graphicsSelected.DrawString(number.ToString(), textFont, br,
                        new Rectangle(0, 0, IMAGELIST_WIDTH, IMAGELIST_HEIGHT), textFormat);
#else
                graphicsSelected.DrawString(number.ToString(), this.Font, br,
                        new Rectangle(0, 0, IMAGELIST_WIDTH, IMAGELIST_HEIGHT), textFormat);

#endif
                string quadrant = QUADRANT_IMAGE_KEY;
                quadrant += i.ToString();
                this.SmallImageList.Images.Add(quadrant, bm2);

                var dummy = SmallImageList.Handle;              //This is dumb, but it has to be here.
            }
        ...
        {
            int i;
            for (i = 0; i < SmallImageList.Images.Count; i++)
            {
                SmallImageList.Images[i].Dispose();
            }
            for (i = 0; i < StateImageList.Images.Count; i++)
            {
                StateImageList.Images[i].Dispose();
            }
