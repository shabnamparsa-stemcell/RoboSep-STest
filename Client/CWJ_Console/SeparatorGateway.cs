...
namespace Tesla.OperatorConsoleControls
{
	...
	public class SeparatorGateway
	{
        ...
        private bool                    myProtocolUpdate = false;
        private bool                    UpdatingSeparatorProtocols = false;

        public Mutex                    myHaltMutex = new Mutex();
        public Mutex                    myUnloadMutex = new Mutex();
        public Mutex                    myPause = new Mutex();
		...
		private void NotifySeparationProtocolTableUpdate(DataTable aSeparationProtocolData)
		{
            ...
            if (UpdatingSeparatorProtocols)
                myProtocolUpdate = true;
		}

        public bool protocolUpdated
        {
            get
            {
                return myProtocolUpdate;
            }
            set
            {
                myProtocolUpdate = value;
            }
        }

        public bool separatorUpdating
        {
            set
            {
                UpdatingSeparatorProtocols = value;
            }
        }
        ...
        public string GetProtocolName(QuadrantId quadrantId)
        {
            IProtocol result = GetProtocol(quadrantId);
        ...
        public IProtocol GetProtocol(QuadrantId quadrantId)
        {
            IProtocol foundProtocol = null;
            if (myChosenProtocols != null)
            {
                foreach(QuadrantId i in Enum.GetValues(typeof(QuadrantId)))
                {
                    if (myChosenProtocols.Contains(i))
                    {
                        int initialQuadrant = (int) (myChosenProtocols[i] as IProtocol).InitialQuadrant;
                        int quadrantCount = (myChosenProtocols[i] as IProtocol).QuadrantCount;
                        if ((int)quadrantId >= initialQuadrant && (int)quadrantId < (initialQuadrant + quadrantCount))
                        {
                            foundProtocol = myChosenProtocols[i] as IProtocol;
                            break;
                        }
                    }
                }
		...
		{
            if (statusMessageValues == null || statusMessageValues.Length == 0)
                return;
            ...
            try
            {
                ...
                {
                    int nLastIndex1 = statusMessage.LastIndexOf('{');
                    int nLastIndex2 = statusMessage.LastIndexOf('}');
                    if (0 <= nLastIndex1 && 0 <= nLastIndex2 && nLastIndex1 < nLastIndex2)
                    {
                        string s = statusMessage.Substring(nLastIndex1 + 1, nLastIndex2 - nLastIndex1 - 1);
                        int n = Convert.ToInt32(s);
                        if (n + 1 == statusMessageValues.Length)
                        {
                ...
                {
                    statusMessage = string.Format("Failed to get status message from Resource Manager for statusMsgCode={0}", statusMsgCode.ToString());
                }
            	...
            	out string protocolName, out ProtocolClass protocolClass, out int initialQuadrant)
		{
            ...
            System.Diagnostics.Debug.WriteLine(String.Format("GetProtocolConsumables called. selectedProtocolIndexNumber = {0}.", selectedProtocolIndexNumber.ToString()));

			try
			{		
				QuadrantId lookupKey = (QuadrantId)myChosenProtocols.GetKey(selectedProtocolIndexNumber);
                System.Diagnostics.Debug.WriteLine(String.Format("myChosenProtocols.GetKey returns QuadrantId = {0}.", lookupKey.ToString()));  // bdr

				protocolConsumables = (ProtocolConsumable[,])myProtocolConsumables[lookupKey];
                if (protocolConsumables == null)
                {
                    System.Diagnostics.Debug.WriteLine(String.Format("protocolConsumables = null."));  
                }
                ...
                System.Diagnostics.Debug.WriteLine(String.Format("protocolName={0}. initialQuadrant={1}.", protocolName, (QuadrantId)initialQuadrant));  
			}
			catch (Exception ex)
			{
				LogFile.LogException(TraceLevel.Error, ex);
...
/*CR*/	public void GetCustomNames(string name, out string[]customNames, out int numQuadrants)
        ...
        public void StartRun(bool isSharing, int[] sharedSectorsTranslation, int[] sharedSectorsProtocolIndex, ReagentLotIdentifiers[][] reagentLotIds)
		{
        ...
        public int SetIgnoreHydraulicSensor(int sw)
        {
            ...
            try
            {
                rtn = mySeparator.SetIgnoreHydraulicSensor(sw);
            }
            catch (Exception ex)
            {
                LogFile.LogException(TraceLevel.Error, ex);
        ...
        public bool GetIgnoreHydraulicSensor()              
        {
            ...
            try
            {
                rtn = mySeparator.GetIgnoreHydraulicSensor();
            }
            catch (Exception ex)
            {
                LogFile.LogException(TraceLevel.Error, ex);
        ...
        public bool IsHydroFluidLow()
        {
            ...
            try
            {
                rtn = mySeparator.IsHydroFluidLow();
            }
            catch (Exception ex)
            {
                LogFile.LogException(TraceLevel.Error, ex);
        ...
        public int InitScanVialBarcode(bool freeaxis)                        //CWJ
        {
            ...
            try
            {
                rtn = mySeparator.InitScanVialBarcode(freeaxis);
            }
            catch (Exception ex)
            {
                LogFile.LogException(TraceLevel.Error, ex);
        ...
        public bool AbortScanVialBarcode()                        //twong
        {
            ...
            try
            {
                rtn = mySeparator.AbortScanVialBarcode();
            }
            catch (Exception ex)
            {
                LogFile.LogException(TraceLevel.Error, ex);
        ...
        public bool IsBarcodeScanningDone()                        //twong
        {
            ...
            try
            {
                rtn = mySeparator.IsBarcodeScanningDone();
            }
            catch (Exception ex)
            {
                LogFile.LogException(TraceLevel.Error, ex);
        ...
        public string GetEndOfRunXMLFullFileName()                 // 09-25-2013 - Sunny added
        {
            ...
            try
            {
                rtn = mySeparator.GetEndOfRunXMLFullFileName();
            }
            catch (Exception ex)
            {
                LogFile.LogException(TraceLevel.Error, ex);
        ...
        private void InitialiseSeparatorConnection()
        {
            ...
            //Debug.WriteLine(String.Format("------------------InitialiseSeparatorConnection ------------------"));

            NameValueCollection nvc = (NameValueCollection)System.Configuration.ConfigurationManager.GetSection("OperatorConsole/SeparatorConnection");
		...
		{
            NameValueCollection nvc = (NameValueCollection)System.Configuration.ConfigurationManager.GetSection("OperatorConsole/ConsoleConfiguration");
			try
			{
        ...
        private void AtSeparatorStateUpdate(SeparatorState newState)
        {
            try
            {
                ...
                System.Diagnostics.Debug.WriteLine(String.Format("--------- SeparatorGateway: AtSeparatorStateUpdate - SeparatorState={0}. ", newState));
					...
					lock(this)
					{
						...
						switch (newState)
						{
							...
							case SeparatorState.PauseCommand:
								...
								break;
							default:
								System.Diagnostics.Debug.WriteLine(String.Format("------------------SeparatorGateway AtSeparatorStateUpdate {0} undefined???", newState));
								break;
						}
					}
				}
			...
			QuadrantId[] quadrantIds, ProtocolConsumable[,] consumables)
		{
            if (quadrantIds == null)
            {
                System.Diagnostics.Debug.WriteLine(String.Format("---------AtProtocolConsumablesUpdate: input parameter is null."));
                return;
            }
			try
			{
				...
				if (quadrantCount != 0)
				{
					lock(this)
					{
						...
						if (consumables == null)
						{
                            System.Diagnostics.Debug.WriteLine(String.Format("---------AtProtocolConsumablesUpdate: given 'consumables' are null."));
								...
								++quadrantId)
							{
								...
								{
                                    System.Diagnostics.Debug.WriteLine(String.Format("---------AtProtocolConsumablesUpdate: remove any previous consumables information for quadrant={0}.", quadrantId));
						...
						{
                            System.Diagnostics.Debug.WriteLine(String.Format("---------AtProtocolConsumablesUpdate: given 'consumables' are not null."));
                        ...
                        System.Diagnostics.Debug.WriteLine(String.Format("---------AtProtocolConsumablesUpdate: update the protocol table (using the newly received sample volumes)."));
		...
		private void RefreshProtocolTableContents(ProtocolMix protocolMix)
		{
            ...
            System.Diagnostics.Debug.WriteLine(String.Format("---------Now update the display"));
        ...
        public string isSharingComboValid(bool isSharing, int[] sharedSectorsTranslation, int[] sharedSectorsProtocolIndex)
        {
            return mySeparator.isSharingComboValid(myBatchRunUserId,
                    isSharing, sharedSectorsTranslation, sharedSectorsProtocolIndex);
        }
    }
}
