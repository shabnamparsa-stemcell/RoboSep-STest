    ...
    public partial class ProgressBar : UserControl
    {
        ...
        private void OverlayRectangle_Paint(object sender, PaintEventArgs e)
        {
            ...
            if (ShowDetails || true)
            {
                ...
                if (ShowElapseTime)
                {
                    e.Graphics.DrawString(elapsedTime, new Font("Arial", 12, FontStyle.Regular),
                        new SolidBrush(Color.Black), new Point(21, (this.Size.Height / 2) + 4), theFormat);
                    //OverlayRectangle.BringToFront();
                }

                // draw text
                theFormat.Alignment = StringAlignment.Center;
                e.Graphics.DrawString(percentCompleted, new Font("Arial", 14, FontStyle.Regular),
                    new SolidBrush(this.ForeColor), new Point((this.Size.Width / 2) + 3, (this.Size.Height / 2) + 3), theFormat);
                theFormat.Alignment = StringAlignment.Near;

                if (ShowElapseTime)
                {
                    e.Graphics.DrawString(elapsedTime, new Font("Arial", 12, FontStyle.Regular),
                        new SolidBrush(this.ForeColor), new Point(20, (this.Size.Height / 2) + 3), theFormat);
                    //OverlayRectangle.BringToFront();
                }
            }
        }
