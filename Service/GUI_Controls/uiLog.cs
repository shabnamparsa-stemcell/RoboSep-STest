...
namespace GUI_Controls
{
    public class uiLog
    {
        ...
        private uiLog(string suffix)
        {
            do
            {
                strLogPath = GetstrLogPath(suffix);
        ...
        {
            if (myLogFile == null)
                myLogFile = new uiLog("UI");
            return myLogFile;
        }
        public static uiLog getServiceInstance()
        {
            if (myLogFile == null)
                myLogFile = new uiLog("Service");
            return myLogFile;
        }

        private string GetstrLogPath(string suffix)
        {
            try
            {
                ...
                strLogPath += logGenTime + "_" + suffix+ ".log";
        ...
        private void InitializeFile(string logPath)
        {
            ...
            {
                // get prompt message
                string msg = GUI_Controls.fromINI.getValue("GUI", "msgLogWriteFail");
                msg = msg == null ? "Unable to write to log file '" : msg;
                msg += logPath + "'";

                RoboMessagePanel err = new RoboMessagePanel(Application.OpenForms[0], msg);
                err.ShowDialog();
            }
        }
        ...
        public static void LOG(object origin, string FTN, LogLevel logLVL, string msg)
        {
            ...
            LOG_helper( mylog,   origin,   FTN,   logLVL,   msg);
        }
        public static string GET_SERVICE_LOG_DIR()
        {
            uiLog mylog = uiLog.getServiceInstance();
            return mylog.strLogPath;
        }
        public static void SERVICE_LOG(object origin, string FTN, LogLevel logLVL, string msg)
        {
            // get current Log file
            uiLog mylog = uiLog.getServiceInstance();
            LOG_helper(mylog, origin, FTN, logLVL, msg);
        ...
        public static void SET_SERVICE_LOG_OUTPUT_LEVEL(LogLevel setLevel)
        {
            // get current Log file
            uiLog mylog = uiLog.getServiceInstance();
        ...
        public static LogLevel GET_SERVICE_LOG_OUTPUT_LEVEL()
        {
            // get current Log file
            uiLog mylog = uiLog.getServiceInstance();

            // set log level
            return mylog.outLevel;
        }

        private static void LOG_helper(uiLog mylog, object origin, string FTN, LogLevel logLVL, string msg)
        {
